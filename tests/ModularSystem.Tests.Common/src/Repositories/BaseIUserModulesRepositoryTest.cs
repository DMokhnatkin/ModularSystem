...
namespace ModularSystem.Tests.Common.Repositories
{
    ...
    public abstract class BaseIUserModulesRepositoryTest
    {
        ...
        public void TestRemoveModule()
        {
            ...
            Assert.IsTrue(UserModulesRepository.GetModules("1").Contains(SampleModules[0]));
            UserModulesRepository.RemoveModule("1", SampleModules[0]);
            Assert.IsFalse(UserModulesRepository.GetModules("1").Contains(SampleModules[0]));

            Assert.IsTrue(UserModulesRepository.GetModules("1").Contains(SampleModules[1]));
            UserModulesRepository.RemoveModule("1", SampleModules[1]);
            Assert.IsFalse(UserModulesRepository.GetModules("1").Contains(SampleModules[1]));

            Assert.IsTrue(UserModulesRepository.GetModules("1").Contains(SampleModules[2]));
            UserModulesRepository.RemoveModule("1", SampleModules[2]);
            Assert.IsFalse(UserModulesRepository.GetModules("1").Contains(SampleModules[2]));
        }
